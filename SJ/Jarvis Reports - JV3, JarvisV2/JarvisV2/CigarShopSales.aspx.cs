...
public partial class CigarShopSales : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        if (!IsPostBack)
        {
            StartDate.Text = DateTime.Now.AddDays(-60).Date.ToShortDateString();
            EndDate.Text = DateTime.Now.AddDays(-1).Date.ToShortDateString();
            var cigSales = CigrteSales.GetCigarSalesData();
        ...
        if (!IsPostBack)
        {
            ...
            if (dtNew != null)
            {
                ...
                {
                    CheckBoxList1.Items[CheckBoxList1.Items.IndexOf(new ListItem(Items.ToString()))].Selected = true;
                }
        ...
        Title = "Cigar Shop Sales";// "Store " + ddlStoreID.SelectedValue + " Cig. Sales";
    ...
    public DataTable GetCouponSalesDetail(int NoOfDays)
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("[Jarvis].[dbo].[Get6MonthCigarSalesCompiled]", connection);
