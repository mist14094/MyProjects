...
public partial class WinBinScan : System.Web.UI.Page
{
    private WinBinMatrix WinBinMatrix = new WinBinMatrix();
    ...
    public static string _SJSQLSystem = ConfigurationManager.ConnectionStrings["SJSQL"].ConnectionString;
    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            var winBinMatrix = WinBinMatrix.GetWinBinData();

            ddlBrand.DataSource = winBinMatrix.Select(
                matrix => new {matrix.StoreName}).Distinct().OrderBy(arg => arg.StoreName);
            ddlBrand.DataTextField = "StoreName";
            ddlBrand.DataValueField = "StoreName";
            ddlBrand.DataBind();
        ...
        if (!IsPostBack)
        {
            ...
            if (dtNew != null)
            {
                CheckBoxList1.DataSource = GetLocationDetail();
                CheckBoxList1.DataTextField = "Desc";
                CheckBoxList1.DataValueField = "Desc";
                CheckBoxList1.DataBind();
               // CheckBoxList1.Items.RemoveAt(CheckBoxList1.Items.IndexOf(new ListItem("Date")));
    ...
    public void SetValues(string GraphType, string Title)
    {
        ...
        if (dtNew != null)
        {
            ...
            {
                if (
                    DateTime.Parse(dtBasedonSelection.Rows[dtBasedonSelection.Rows.Count - 1]["Date"].ToString())
                        .Date ==
                    DateTime.Now.Date)
                {
    ...
    public DataTable GetWinBinScan()
    {
        DataTable allData = new DataTable();
        SqlConnection connection = new SqlConnection(_RFIDSystem);
        try
        {
            SqlCommand cmd = new SqlCommand("GetWinBinScan", connection);
 ...
 public DataTable GetLocationDetail()
    {
        DataTable allData = new DataTable();
        SqlConnection connection = new SqlConnection(_RFIDSystem);
        try
        {
            SqlCommand cmd = new SqlCommand("select * from [WinBinMatrix]", connection);
    ...
    public void SetCigValues()
    {
        ...
        var cigarMatrix = WinBinMatrix.GetWinBinData();

        if (ddlBrand.SelectedValue != "ALL")
        {
            cigarMatrix = cigarMatrix.Where(matrix => matrix.StoreName == ddlBrand.SelectedValue).ToList();
        }

        for (int i = 0; i < CheckBoxList1.Items.Count; i++)
        {
            if (cigarMatrix.Count(matrix => matrix.Desc == CheckBoxList1.Items[i].ToString()) != 0)
            {
                CheckBoxList1.Items[i].Selected = true;
            }
            else
