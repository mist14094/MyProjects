...
namespace AdWeb
{
    public partial class Monitor : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect(string.Format("JustCountMonitor.aspx?DeviceID={0}", 9));
        }

        protected void btnZipLine_Click(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("JustCountMonitor.aspx?DeviceID={0}", 10));
        }

        protected void btnJumpZone_Click(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("JustCountMonitor.aspx?DeviceID={0}", 13));
        }

        protected void btnExtraAct1InCount_Click(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("JustCountMonitor.aspx?DeviceID={0}", 14));
        }

        protected void btnKidZone_Click(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("JustCountMonitor.aspx?DeviceID={0}", 12));
        }

        protected void btnBullRide_Click(object sender, EventArgs e)
        {
            Response.Redirect(string.Format("JustCountMonitor.aspx?DeviceID={0}", 11));
        }
