...
{
    /// <summary>
    /// This state the current proximity status(within range/out of range).    
    /// </summary>
    ...
    /// <summary>
    /// Proximity range status event arguments
    /// </summary>
    
    [Serializable]
    public class ProximityEventArgs : KTComponentEventArgs
    {

        #region Attributes
		
        private ProximityRange currentRangeStatus = ProximityRange.OutOfRange;
        UInt16 minProxOnReader=0;
        UInt16 maxProxOnReader = 0;
        UInt16 currentProximity = 0;

	    #endregion

        #region Constructors

        public ProximityEventArgs(ProximityRange rangeStatus,UInt16 curProximity , UInt16 minRange, 
            UInt16 maxRange, IKTComponent component, string eventId)
            : base(component, eventId)
        {
            this.currentRangeStatus = rangeStatus;
            this.currentProximity = curProximity;
            this.minProxOnReader = minRange;
            this.maxProxOnReader = maxRange;
        } 
        #endregion

        #region Properties

        public UInt16 MinProximityOnReader
        {
            get
            {
                return minProxOnReader;
            }
        }

        public UInt16 MaxProximityOnReader
        {
            get
            {
                return maxProxOnReader;
            }
        }

        public ProximityRange RangeStatus
        {
            get
            {
                return currentRangeStatus;
            }
        }

        public UInt16 CurrentProximityValue
        {
            get
            {
