...
Public Class MainWindow
    ...
    Public Sub New()
        ...
        MyShortcut = wsh.CreateShortcut("C:\WinbinExternal.Lnk")

        ' Set shortcut object properties and save it

        MyShortcut.TargetPath = wsh.ExpandEnvironmentStrings(Assembly.GetExecutingAssembly().CodeBase)

        MyShortcut.WorkingDirectory = wsh.ExpandEnvironmentStrings(Assembly.GetExecutingAssembly().CodeBase.Substring(0, Assembly.GetExecutingAssembly().CodeBase.Length - 18))
    ...
    Private Sub retrieveLogo()
        Try
            ...
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getLogo.php")
    ...
    Protected Sub loadContent()
        ...
        Try
            ...
            Try
                Dim webpost As New WebPostRequest("http://192.168.1.5/client/getFeaturedProducts.php")
                webpost.Add("macaddr", terminal_info.MacAddress)
                webpost.Add("sechash", secHash)
            ...
            If elements.Count() > 5 Then
                ...
                If (elements(2).Contains(".avi")) Then
                    Dim imageAddress As String = String.Empty
                    Dim filename As String = String.Empty
                    Try
                        Dim fileReader As New WebClient()

                        imageAddress = elements(2)
                        ...
                        elements(2) = "C:\" + filename
                    Catch ex As Exception
                        MessageBox.Show(ex.InnerException.Message)
                    End Try
                End If

                If (elements(5).Contains(".avi")) Then
                    Dim imageAddress As String = String.Empty
                    Dim filename As String = String.Empty
                    Try
                        Dim fileReader As New WebClient()

                        imageAddress = elements(5)
                        ...
                        elements(5) = "C:\" + filename
                    Catch ex As Exception
                        MessageBox.Show(ex.InnerException.Message)
                    End Try
                End If

                If (elements(8).Contains(".avi")) Then
                    Dim imageAddress As String = String.Empty
                    Dim filename As String = String.Empty
                    Try
                        Dim fileReader As New WebClient()

                        imageAddress = elements(8)
    ...
    Private Function getPromoType() As PromoInfo
        ...
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getPromoType.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
            webpost.Add("sechash", secHash)
            response = webpost.GetResponse()
        Catch ex As Exception
    ...
    Private Function getEarnedPoints(curcust As Customer) As Integer
        ...
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getEarnedPoints.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
    ...
    Private Sub cardScanned(cardnumber As String)
        ...
        If currentCustomer.located Then
            ...
            Dim req_update As Boolean = False
            Dim good_to_go As Boolean = True

            If currentCustomer.Email.Length = 0 And terminal_info.ReqEmail Then
                req_update = True
            End If

            If currentCustomer.Email.Length = 0 And terminal_info.ReqEmail Then
                Dim enteremail As New EnterEmail(terminal_info, logo)
                Dim result As Boolean = enteremail.ShowDialog()
            ...
            If promoinfo.promotype = "POINTS" And good_to_go Then
    ...
    Private Function getCustomerInfo(ByVal cardnumber As String) As Customer
        ...
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getCustInfoCard.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
    ...
    Private Function getCustomerInfo(State As String, IDNum As String) As Customer
        ...
        Dim response As String = ""
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getCustInfoID.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
    ...
    Private Function createEntry(customer As Customer) As List(Of String)
        ...
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/createEntry.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
    ...
    Private Function createEntries(customer As Customer, numentries As Integer) As List(Of String)
        ...
        Dim resplist As New List(Of String)()
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/createEntries.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
            webpost.Add("custid", customer.CustID)
    ...
    Public Sub createNewCustomer(curCustomer As Customer)
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/createNewCustomer.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
    ...
    Private Sub getStoreNum()
        ...
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getStoreNumber.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
            webpost.Add("sechash", secHash)
            response = webpost.GetResponse()
        Catch ex As Exception
    ...
    Private Sub getTermRequirements()
        ...
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/getTerminalReqs.php")
            webpost.Add("macaddr", terminal_info.MacAddress)
            webpost.Add("sechash", secHash)
            response = webpost.GetResponse()
        Catch ex As Exception
    ...
    Private Sub updateCustomer(currentCustomer As Customer)
        Dim response As String = ""
        Try
            Dim webpost As New WebPostRequest("http://192.168.1.5/client/updateCustEmail.php")
            webpost.Add("cardnum", currentCustomer.CardNum)
            webpost.Add("email", currentCustomer.Email)
            response = webpost.GetResponse()
        Catch
        End Try
    End Sub
